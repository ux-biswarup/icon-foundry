...
for (const file of files) {
  ...
  const svg = result.issues.some((i) => i.rule === "compose") ? "" : renderSvg(compose(spec, language), language);
  if (svg) writeFileSync(join(out, `${spec.name}.svg`), svg);

  const status = result.valid ? "valid" : "INVALID";
  if (!result.valid) failures++;
  console.log(`${status.padEnd(8)} ${basename(file).padEnd(36)} issues=${result.issues.length}`);
